...
export function useIntersectionLoadMore(options: UseIntersectionLoadMoreOptions) {
    const { rootSelector, targetSelector, onLoadMore, context, skipInitial = true } = options
    ...
    const start = () => {
        ...
        observer
            .relativeTo(rootSelector)
            .observe(targetSelector, (res) => {
                const isIntersecting = !!(res?.intersectionRatio && res.intersectionRatio > 0)
